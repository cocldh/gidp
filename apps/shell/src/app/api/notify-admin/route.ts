...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.RESEND_FROM_EMAIL ?? 'GIDP <onboarding@resend.dev>',
        to: [adminEmail],
        subject: '[GIDP] New User Sign-up — Approval Required',
        html: `
          <div style="font-family:sans-serif;max-width:480px;">
            ...
            <p style="color:#6b7280;font-size:12px;margin-top:24px;">GIDP — GS Instrument Data Platform</p>
          </div>
        `,
      }),
    })
