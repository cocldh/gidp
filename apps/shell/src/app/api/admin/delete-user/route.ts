...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (e1) console.error('[delete-user] user_project_role:', e1.message)

    const { error: e2 } = await adminClient.from('user_profile').delete().eq('id', userId)
    if (e2) console.error('[delete-user] user_profile:', e2.message)

    const { error: e3 } = await adminClient.auth.admin.deleteUser(userId)
    if (e3) {
      console.error('[delete-user] auth.admin.deleteUser:', e3.message)
