...
type GlobalRole = 'Pending' | 'Active' | 'Admin'
type ProjectRole = 'ProjectAdmin' | 'Editor' | 'Viewer'
...
export default function UsersPage() {
  ...
  async function loadUsers() {
    setLoading(true)
    const { data } = await supabase.from('user_profile').select('*').order('created_at')
  ...
  async function changeGlobalRole(userId: string, newRole: GlobalRole) {
    const { error } = await supabase.from('user_profile').update({ role: newRole }).eq('id', userId)
  ...
  async function deleteUser(userId: string, email: string, label: string) {
    ...
    if (!res.ok) {
      ...
      loadUsers()
      if (selectedUserId === userId) { setSelectedUserId(null); setProjectRoles([]) }
    }
    setActionUser(null)
  }
  ...
  async function saveDisplayName(userId: string) {
    ...
    else { setEditingDisplayNameId(null); setEditingDisplayNameValue(''); await loadUsers() }
  ...
  async function saveUsername(userId: string) {
    ...
    if (!res.ok) { const { error } = await res.json(); setUsernameError(error ?? 'Error') }
    else { setEditingUsernameId(null); setEditingUsernameValue(''); await loadUsers() }
  ...
  async function assignProjectRole() {
    ...
    const { error } = await supabase.from('user_project_role').upsert(
      { user_id: selectedUserId, project_id: assignProjectId, role: assignRole, assigned_by: myId },
  ...
  async function removeProjectRole(uprId: number) {
    const { error } = await supabase.from('user_project_role').delete().eq('id', uprId)
    if (error) alert(`Error: ${error.message}`)
  ...
  async function updateProjectRole(uprId: number, newRole: ProjectRole) {
    const { error } = await supabase.from('user_project_role').update({ role: newRole }).eq('id', uprId)
    if (error) alert(`Error: ${error.message}`)
    else if (selectedUserId) await loadProjectRoles(selectedUserId)
  }
      ...
      <header className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="/" className="text-sm text-gray-500 hover:text-gray-700">← GIDP</a>
        ...
        {loading ? (
          ...
          <>
                    ...
                    <tbody className="divide-y divide-orange-100">
                      {pendingUsers.map(user => (
                        <tr key={user.id}>
                          ...
                          <td className="px-4 py-3 text-gray-500 text-xs">{new Date(user.created_at).toLocaleString()}</td>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                          ...
                          {editingDisplayNameId === user.id ? (
                            <div className="flex items-center gap-1 min-w-32">
                              <input
                                ...
                                onKeyDown={e => { if (e.key === 'Enter') saveDisplayName(user.id); if (e.key === 'Escape') { setEditingDisplayNameId(null) } }}
                                autoFocus
                                className="px-2 py-0.5 border border-blue-400 rounded text-xs w-28 focus:outline-none"
                                placeholder="display name"
                              />
                              <button onClick={() => saveDisplayName(user.id)} disabled={displayNameLoading} className="px-2 py-0.5 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 disabled:opacity-50">{displayNameLoading ? '...' : '저장'}</button>
                              <button onClick={() => setEditingDisplayNameId(null)} className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded hover:bg-gray-300">취소</button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 group">
                              <span className={`text-xs ${user.display_name ? 'text-gray-700' : 'text-gray-300'}`}>{user.display_name || '—'}</span>
                              <button onClick={() => { setEditingDisplayNameId(user.id); setEditingDisplayNameValue(user.display_name ?? '') }} className="opacity-0 group-hover:opacity-100 px-1 py-0.5 text-xs text-blue-500 hover:text-blue-700" title="display name 편집">✎</button>
                            </div>
                          )}
                        </td>
                            ...
                            <div className="flex flex-col gap-1 min-w-32">
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  onKeyDown={e => { if (e.key === 'Enter') saveUsername(user.id); if (e.key === 'Escape') setEditingUsernameId(null) }}
                                  autoFocus
                                  className="px-2 py-0.5 border border-blue-400 rounded text-xs font-mono w-28 focus:outline-none"
                                  placeholder="username"
                                />
                                <button onClick={() => saveUsername(user.id)} disabled={usernameLoading} className="px-2 py-0.5 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 disabled:opacity-50">{usernameLoading ? '...' : '저장'}</button>
                                <button onClick={() => setEditingUsernameId(null)} className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded hover:bg-gray-300">취소</button>
                              </div>
                              {usernameError && <span className="text-xs text-red-500">{usernameError}</span>}
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 group">
                              <span className={`font-mono text-xs ${user.username ? 'text-gray-700' : 'text-gray-300'}`}>{user.username || '—'}</span>
                              <button onClick={() => { setEditingUsernameId(user.id); setEditingUsernameValue(user.username ?? ''); setUsernameError('') }} className="opacity-0 group-hover:opacity-100 px-1 py-0.5 text-xs text-blue-500 hover:text-blue-700" title="username 편집">✎</button>
                            </div>
                          )}
                        </td>
                    ...
                    {activeUsers.length === 0 && (
                      <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-400">No active users</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div>
              ...
              {!selectedUser ? (
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center text-gray-400 text-sm">위 목록에서 사용자를 선택하세요.</div>
              ) : (
                <div className="bg-white rounded-lg shadow p-4 space-y-4">
                        ...
                        {projectRoles.map(pr => (
                          <tr key={pr.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-3 py-2">
                              <select value={pr.role} onChange={e => updateProjectRole(pr.id, e.target.value as ProjectRole)} className="px-2 py-0.5 border border-gray-300 rounded text-xs">
                                <option value="ProjectAdmin">ProjectAdmin</option>
                                <option value="Editor">Editor</option>
                                <option value="Viewer">Viewer</option>
                              </select>
                            </td>
                            <td className="px-3 py-2 text-gray-400 text-xs">{new Date(pr.assigned_at).toLocaleDateString()}</td>
                            <td className="px-3 py-2">
                              <button onClick={() => removeProjectRole(pr.id)} className="px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded hover:bg-red-200 border border-red-200">Remove</button>
                            </td>
                          </tr>
                        ))}
                        {projectRoles.length === 0 && (
                          <tr><td colSpan={4} className="px-3 py-4 text-center text-gray-400">이 사용자에게 할당된 프로젝트 역할이 없습니다.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                  <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
                    ...
                    <select value={assignProjectId} onChange={e => setAssignProjectId(e.target.value ? Number(e.target.value) : '')} className="px-2 py-1.5 border border-gray-300 rounded text-sm flex-1 max-w-48">
                      <option value="">프로젝트 선택...</option>
                      {projects.map(p => <option key={p.project_id} value={p.project_id}>{p.project_name}</option>)}
                    </select>
                    <select value={assignRole} onChange={e => setAssignRole(e.target.value as ProjectRole)} className="px-2 py-1.5 border border-gray-300 rounded text-sm">
                      <option value="ProjectAdmin">ProjectAdmin</option>
                      <option value="Editor">Editor</option>
                      <option value="Viewer">Viewer</option>
                    </select>
                    <button onClick={assignProjectRole} disabled={!assignProjectId || assignLoading} className="px-3 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 disabled:opacity-50 font-medium whitespace-nowrap">
