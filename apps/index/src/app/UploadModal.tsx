...
export default function UploadModal({ open, onClose, onUploadComplete }: UploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const maxOrder = existingCols && existingCols.length > 0
        ? Math.max(...existingCols.map((c: any) => c.order_index))
        : -1;

      const newCols = headers
        .filter((h) => !existingSet.has(h))
        .map((h, i) => ({ column_name: h, order_index: maxOrder + 1 + i }));

      if (newCols.length > 0) {
        await supabase.from("index_columns").insert(newCols);
      }

      // 3. Load existing records to build TAGNUMBER -> id map
      setStatus("uploading");
      ...
      // 4. Classify rows into inserts / updates
      const toInsert: { data: Record<string, any> }[] = [];
      const toUpdate: { id: number; data: Record<string, any> }[] = [];

      for (const row of rows) {
        const tag = row[PK_FIELD] != null ? String(row[PK_FIELD]) : null;
        if (tag && tagToId.has(tag)) {
          toUpdate.push({ id: tagToId.get(tag)!, data: row });
        } else {
          toInsert.push({ data: row });
        }
      }
      ...
      for (let i = 0; i < toInsert.length; i += BATCH_SIZE) {
        ...
        const { error } = await supabase.from("index_records").insert(batch);
      ...
      for (let i = 0; i < toUpdate.length; i += BATCH_SIZE) {
        ...
        await Promise.all(
          batch.map((r) =>
            supabase.from("index_records").update({ data: r.data }).eq("id", r.id)
          )
        );
        processed += batch.length;
        setProgress({ processed, total: rows.length });
      ...
      localStorage.removeItem("index_col_widths");
      setStatus("done");
      onUploadComplete();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={handleClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-slate-700">
          ...
          <button onClick={handleClose} disabled={isUploading} className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-200 disabled:opacity-40">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          {(isUploading || status === "done") && (
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs text-gray-500 dark:text-slate-400">
                <span>{status === "parsing" ? "파일 파싱 중..." : status === "done" ? "완료" : `${progress.processed.toLocaleString()} / ${progress.total.toLocaleString()} 행`}</span>
                <span>{percent}%</span>
              </div>
          ...
          {status === "done" && (
            <div className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg px-4 py-3">
              ...
              <span>완료: <strong>{summary.inserted}</strong>건 삽입, <strong>{summary.updated}</strong>건 업데이트</span>
            </div>
          )}
