...
export default async function IisClassificationPage() {
  ...
  const [{ data: templates }, { data: fkSummary, error: fkErr }] = await Promise.all([
    supabase
      .schema('drawings')
  ...
  if (fkErr) {
    return (
      <div className="min-h-screen p-8">
        <h1 className="text-xl font-bold mb-2 text-red-600">Failed to load function keys</h1>
        <pre className="text-xs bg-red-50 p-3 rounded">{fkErr.message}</pre>
      </div>
    )
  }
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        <p className="text-sm text-gray-500 mb-6">
          Tag number(<code className="font-mono">D44-UUU-FK-SSSS</code>) 의 function key (FK) 를 기준으로 각 태그를 어떤 IIS 템플릿으로 라우팅할지 정의합니다.
          우선순위(priority) 가 낮을수록 먼저 적용되며, 가장 먼저 매칭되는 규칙이 사용됩니다. 우측 미리보기로 실제 라우팅 결과를 확인하세요.
        </p>
        <ClassificationEditor
          projectId={projectId}
          templates={(templates ?? []) as TemplateRow[]}
          functionKeys={(fkSummary ?? []) as FunctionKeyRow[]}
        />
      </main>
    </div>
  )
