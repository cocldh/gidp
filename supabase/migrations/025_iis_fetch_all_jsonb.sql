-- PostgREST applies an implicit LIMIT (max-rows config or supabase-js default
-- Range header) to RETURNS TABLE / SETOF RPCs. iis_fetch_all_tags was being
-- silently truncated to 2000 rows in postgres logs, breaking the auto/all
-- flows that need every committed tag.
--
-- Wrapping the result in jsonb_agg returns a single scalar row, which PostgREST
-- cannot range-limit. Caller parses the array in JS.
...
GRANT EXECUTE ON FUNCTION drawings.iis_fetch_all_tags_jsonb(int, text, text[]) TO authenticated;
